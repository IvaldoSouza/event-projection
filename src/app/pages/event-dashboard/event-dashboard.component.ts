...
export class EventDashboardComponent implements OnInit {
    ...
    ) {this.orderedDays = this.getOrderedDays();}
        ...
        this.cycles.forEach(cycle => {
            ...
            cycle.structure.forEach(dayStructure => {
                ...
                keys.forEach(type => {
                    ...
                    result[dayStructure.day][type] = (result[dayStructure.day][type] ?? 0) + (extra * multiplier);
                });
    ...
    updateChart(keys: EventType[], result: { [day: number]: { [type: string]: number } }) {
        const series = keys.map(type => ({
            name: type,
            data: this.orderedDays.map(day => result[day]?.[type] ?? 0)
        }));
    ...
    getOrderedDays(): number[] {
        const dayOfWeek = new Date().getDay();
        
        if (dayOfWeek === 0 || dayOfWeek === 6) return [1, 2, 3, 4, 5];
        
        const today = dayOfWeek === 0 ? 1 : dayOfWeek;
        const result: number[] = [];
        result.push(today);
        
        for (let i = 1; i < 5; i++) {
            const nextDay = ((today + i - 1) % 5) + 1;
            result.push(nextDay);
        }
